...
from config.settings import APPLICATIONS_JSON_PATH


class ApplicationStorage:
    def __init__(self, path: Path = APPLICATIONS_JSON_PATH):
        self.path = path
    ...
    def update(self, username: str, record_id: str, updates: dict) -> dict | None:
        ...
        for i, r in enumerate(records):
            if r.get("id") == record_id:
                records[i] = {**r, **updates, "id": record_id, "updated_at": datetime.now().isoformat()}
